...
class CriticNetwork:
    def __init__(self, state_size, learning_rate, session, env, envs_to_load, num_of_h_layers=1, name='critic_network'):
        ...
        with tf.variable_scope(name):
            ...
            saved_models = self.load_critic_model()
    ...
    def load_critic_model(self):
        ...
        for env in self.envs_to_load:
            ...
            for l in range(1, self.num_of_h_layers + 1):
                layer_name = 'h' + str(l)
                critic_bias = np.loadtxt(full_path + "_critic_" + layer_name + "_bias_section2.npy", delimiter=',')
                critic_weights = np.loadtxt(full_path + "_critic_" + layer_name + "_weights_section2.npy",
                                            delimiter=',')
                bias = tf.constant_initializer(critic_bias)
                weights = tf.constant_initializer(critic_weights)
                h = [weights, bias]
                h_layers.append(h)
    ...
    def save_critic_model(self):
        ...
        for l in range(1, self.num_of_h_layers + 1):
            layer_name = 'h' + str(l)
            critic_bias = self.sess.run(
                tf.get_default_graph().get_tensor_by_name(self.name + "/" + layer_name + "/bias:0"))
            np.savetxt(full_path + "_critic_" + layer_name + "_bias.npy", critic_bias, delimiter=',')
            critic_weights = self.sess.run(
                tf.get_default_graph().get_tensor_by_name(self.name + "/" + layer_name + "/kernel:0"))
            np.savetxt(full_path + "_critic_" + layer_name + "_weights.npy", critic_weights, delimiter=',')
