...
for game in game_list:
    ...
    if game == 'MountainCarContinuous-v0':
        ...
        actor = ActorNetworkRegression(state_size, action_size, lr_actor, scaler, sess, game)
    else:
        actor = ActorNetwork(state_size, action_size, lr_actor, sess, game)

    critic = CriticNetwork(state_size, lr_critic, sess, game)
    ...
    with sess:
        ...
        for episode in range(max_episodes):
            state = env.reset()
            state = np.pad(state, (0, state_size - len(state)))  # pad with 0
            ...
            for step in range(max_steps):
                ...
                next_state = np.pad(next_state, (0, state_size - len(next_state)))  # pad with 0
                ...
                if done:
                    ...
                    if episode <= 98:
                        ...
                        if game == 'MountainCarContinuous-v0' and average_rewards >= 90:
                            print(' Solved at episode: ' + str(episode))
                            solved = True
                    break
                state = next_state
    ...
    critic_loss = np.array(critic_loss).reshape(
        -1, )  # fix bug in Mountain env, createes shape (None,) instead of (None,1,1,1)
