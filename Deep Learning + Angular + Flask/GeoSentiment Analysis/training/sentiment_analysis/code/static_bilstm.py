...
class BiLSTM:
    ...
    def build_model_graph(self, word_vectors_shape):
        with tf.name_scope("Input"):
            ...
            self.labels_placeholder = tf.placeholder(dtype=tf.int32, shape=[None, self.config.classes_num], name='labels')
        ...
        self.learning_rate = tf.train.inverse_time_decay(self.config.learning_rate, self.global_step,1, 1, name="learning_rate")
        ...
        with tf.variable_scope("BiLSTM"):
            if self.config.layers == 1:
                ...
                forward = tf.contrib.rnn.MultiRNNCell([self.lstm_cell_with_dropout() for _ in range(self.config.layers)])
                backward = tf.contrib.rnn.MultiRNNCell([self.lstm_cell_with_dropout() for _ in range(self.config.layers)])
            ...
            self.weights = tf.get_variable('weights', shape=[ 2 * self.config.hidden_units, self.config.classes_num],
                                     initializer=tf.contrib.layers.xavier_initializer(uniform=True,seed=50,dtype=tf.float32))
    ...
    def build_loss_graph(self):
        ...
        with tf.name_scope("Loss"):
            cross_entropy = tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.predictions, labels=self.labels_placeholder)
...
def train(data_handler, bi_lstm, sess, writer, train_saver, valid_saver, model_train_path, model_valid_path):
    ...
    for ep in range(bi_lstm.config.epochs_num):
        ...
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        train_avg_loss, train_avg_acc = bi_lstm.run_epoch(sess, batch_size, x_train, y_train, [bi_lstm.optimizer]+metric_tensors, dropout_train)
...
def test(data_handler, bi_lstm, sess):
    ...
    x_test = data_handler.ids_matrix["test_matrix"]
    y_test = data_handler.labels["test"]
    test_avg_loss, test_avg_acc = bi_lstm.run_epoch(sess, bi_lstm.config.batch_size, x_test, y_test, metric_tensors, 1, False)
    print("test avg loss: ", test_avg_loss, 'test avg acc: ', test_avg_acc)
