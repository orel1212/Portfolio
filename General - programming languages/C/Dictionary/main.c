...
#define WORD 81
#define definition 201
#include<stdlib.h>
#include<stdio.h>
#include<string.h>
void FreeMemory(char*** ptr, int size);//function that free the dynamic memory
void PrintWAndD(char*** ptr, int wordindex);//function that print the word and the definitions to the word
...
char** InsertValues(char*** ptr, int size, int index);//function that insert to each ptr[i] modified ptr**
void Dictionary();
int main()
{
...
void Dictionary()
{
	...
	printf("*Note, the dictionary is not supporting a duplicate words/definitions!\n");
	...
	if (ptr != NULL)//if ptr isn't null we can continue
	{
		for (i = 0; i < size; i++)
		{
			...
			ptr[i] = InsertValues(&ptr[i], size, i);//we insert each place in the ***ptr a **ptr when we send to Insertvalue the place, and insertvalue return ready **ptr
		}
		for (i = 0; i < size; i++)
		{
		...
		do//until the user enters exit, he can continue search for words
		{
			...
			if (strcmp(str, "exit") != 0)//if the string the user enters isn't exit
			{
				...
				if (BinarySearch(ptr, 0, size, size, str) == -1)//if after binary search -1 is returned, it mean that the word he entered not exist
					...
					PrintWAndD(ptr, BinarySearch(ptr, 0, size, size, str));//if the word exist, printWandD will print the word and the definitions, and how many definitions
			}
...
char** InsertValues(char*** ptr, int size, int index)
{
	char ch[WORD], chd[definition];
	int sizeod, j;
	printf("Enter how many definitions you want to enter to the dictionary for the word %d \n", index + 1);
	scanf_s("%d", &sizeod);
	getchar();
	ptr = (char**)malloc((sizeod + 2) * sizeof(char*));//+2 because we need '\0' and 1 more field for the word, besides the definitions 
	if (ptr != NULL)
	{
...
{
	int j, count;
	for (count = 0; ptr[wordindex][count] != '\0'; count++);//to count how many definitions we have
	printf("There are %d definitions to the word you chose\n", count - 1); //count-1 because of '\0'
	printf("The word is: ");
	puts(ptr[wordindex][0]);//to print the word
	printf("\n");
	j = 1;//to start printing from definitions
	while (ptr[wordindex][j] != '\0')//if we're not reached yet '\0'
	{
		printf("The %d definition is :", j);
		puts(ptr[wordindex][j]);//print the definition
		printf("\n");
		j++;
	}
}
void FreeMemory(char*** ptr, int size)
{
	...
	for (i = 0; i < size; i++)
	{
		for (j = 0; ptr[i][j] != NULL; j++)
		{
			free(ptr[i][j]);//we free firstly every word and definition at the **
		}
