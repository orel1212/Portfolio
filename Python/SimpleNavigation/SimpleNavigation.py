...
sectionsList = (
(0, 1, 1), (0, 2, 1), (1, 3, 0), (3, 4, 0), (4, 5, 0), (5, 6, 0), (7, 6, 1), (6, 8, 0), (7, 9, 0), (9, 10, 1),
(10, 0, 0))
import random
...
class Navigation():
        ...
        for i in range(0, len(sectionsList)):
            if sectionsList[i][2] == 1:
                Navigation.sections.append(
                    DirectionSection(Navigation.nodes[sectionsList[i][0]], Navigation.nodes[sectionsList[i][1]], True))
            else:
                Navigation.sections.append(
                    DirectionSection(Navigation.nodes[sectionsList[i][0]], Navigation.nodes[sectionsList[i][1]], False))
            if transport == 'walk':
                self.userSpeed = 6
            elif transport == 'car':
    ...
    def navigate(self, id1, id2):
        def calcRouteTime(start, end):
            def searchSection(id1, id2):
                for s in Navigation.sections:
                    ...
                    if ((s.n1).id == id1 and (s.n2).id == id2) or (
                            (s.n2).id == id1 and (s.n1).id == id2 and s.Direction == False):
                        return s
        ...
        if id1 > Navigation.nodes[len(Navigation.nodes) - 1].id or id1 < Navigation.nodes[0].id or id2 > \
                Navigation.nodes[len(Navigation.nodes) - 1].id or id2 < Navigation.nodes[0].id or id1 >= id2:
