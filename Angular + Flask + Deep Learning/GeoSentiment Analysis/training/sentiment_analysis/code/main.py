...
def main():
    ...
    if not flags["load_train"] and not flags["load_valid"]:
        ...
        static_bilstm.train(data_handler,bi_lstm, sess, writer, train_saver, valid_saver, model_train_path, model_valid_path)
        writer.close()
    else:
        ...
        if not flags["test_model"] and flags["serving_version"] == None:
            ...
            if flags["serving_version"] != None:
                static_bilstm.save_prediction_graph(sess, bi_lstm, config_util.export_path_base, flags["serving_version"])
