...
class GeolocationPrediction:
    ...
    def get_predictions(self, host, port, ids_text, ids_misc):
        ...
        result = stub.Predict(request, 600) # 5 seconds
    ...
    def geolocation_preprocess(self, tweet):
        ...
        if tweet.user_desc is not None:
            desc = tokenizer.tokenize(tweet.user_desc)
        else:
            desc = tokenizer.tokenize("")
        ...
        if tweet.user_profile_location is not None:
            profile_location = tokenizer.tokenize(tweet.user_profile_location)
        else: 
            profile_location = tokenizer.tokenize("")
        ...
        try:
            ...
            first_minute_digit = int(time.strftime('%M', time.strptime(tweet.posting_time,'%a %b %d %H:%M:%S +0000 %Y'))[0])
    ...
    def coordinates_to_location_conversion(self,tweets):
        coordinates_list = [tweet.coordinates for tweet in tweets]
        labels = rg.search(coordinates_list)
        for i, label in enumerate(labels):
            #tweets[i].set_country(label["cc"])
            tweets[i].country = label["cc"]
    ...
    def predict_tweets_geolocation(self, tweets, host, port):
        ...
        ids_text = self.create_ids_text(tweets)
        ids_misc = self.create_misc(tweets)
