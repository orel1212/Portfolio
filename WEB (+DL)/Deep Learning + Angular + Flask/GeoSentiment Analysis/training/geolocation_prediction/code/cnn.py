...
class CNN():
    ...
    def build_train_graph(self, word_vectors_shape):
        with tf.variable_scope("Model"):
            with tf.name_scope("Input"):
                self.input_content_placeholder = tf.placeholder(tf.int32, [None, self.config.sequence_content],
                                                                name="input_content")
                self.input_description_placeholder = tf.placeholder(tf.int32, [None, self.config.sequence_description],
                                                                    name="input_description")
                self.input_username_placeholder = tf.placeholder(tf.int32, [None, self.config.sequence_username],
                                                                 name="input_username")
                self.input_profile_location_placeholder = tf.placeholder(tf.int32,
                                                                         [None, self.config.sequence_profile_location],
                                                                         name="input_profile_location")
            ...
            def build_conv_layers(config, embed_input_list):
                ...
                with tf.name_scope("Conv_layer"):
                    for i, filter_size in enumerate(config.filter_sizes):
                        ...
                        h_description = tf.nn.relu(tf.nn.bias_add(conv_description, filter_bias),
                                                   name="relu_description")
                        h_username = tf.nn.relu(tf.nn.bias_add(conv_username, filter_bias), name="relu_username")
                        h_profile_location = tf.nn.relu(tf.nn.bias_add(conv_profile_location, filter_bias),
                                                        name="relu_profile_location")
            ...
            description_pool_flat = build_h_pool(output_dict["description"], self.config.filter_num,
            ...
            profile_location_pool_flat = build_h_pool(output_dict["profile_location"], self.config.filter_num,
                                                      self.config.filter_sizes)

            theta = tf.concat(
                [content_pool_flat, description_pool_flat, username_pool_flat, profile_location_pool_flat], 1)
            ...
            with tf.name_scope("Output"):
                ...
                init = tf.truncated_normal(
                    [len(self.config.filter_sizes) * self.config.filter_num * num_of_text_fields + sum(depth_list),
                     self.config.num_classes], stddev=0.1)
    ...
    def run_epoch(self, sess, batch_size, x_text, x_misc, y, tensors, dropout, train_flag=True):
        ...
        def prepare_batch():
            ...
            return [batch_input_content, batch_input_desc, batch_input_username, batch_input_profile_location,
                    batch_inputs_misc], batch_labels
...
def train(data_handler, cnn_model, sess, writer, train_saver, valid_saver, model_train_path, model_valid_path):
    ...
    for ep in range(cnn_model.config.epochs_num):
        ...
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        train_avg_loss, train_avg_acc = cnn_model.run_epoch(sess, batch_size, x_train[0], x_train[1], y_train,
                                                            [cnn_model.optimizer] + metric_tensors, dropout_train)
        print("ep: ", step, ' train avg loss: ', train_avg_loss, 'train avg acc: ', train_avg_acc)
        valid_avg_loss, valid_avg_acc = cnn_model.run_epoch(sess, batch_size, x_valid[0], x_valid[1], y_valid,
                                                            metric_tensors, 1, False)
        print("ep: ", step, ' valid avg loss: ', valid_avg_loss, 'valid avg acc: ', valid_avg_acc)
        test_avg_loss, test_avg_acc = cnn_model.run_epoch(sess, batch_size, x_test[0], x_test[1], y_test,
                                                          metric_tensors, 1, False)
    ...
    test_avg_loss, test_avg_acc = cnn_model.run_epoch(sess, batch_size, x_test[0], x_test[1], y_test, metric_tensors, 1,
                                                      False)
...
def test(data_handler, cnn_model, sess):
    ...
    x_test = [data_handler.ids_text["test"], data_handler.ids_misc["test"]]
    y_test = data_handler.labels["test"]
    test_avg_loss, test_avg_acc = cnn_model.run_epoch(sess, cnn_model.config.batch_size, x_test[0], x_test[1], y_test,
                                                      metric_tensors, 1, False)
    print("test avg loss: ", test_avg_loss, 'test avg acc: ', test_avg_acc)
